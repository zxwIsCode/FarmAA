...
- (void)updateContent
{
    ...
    for (int i = 0; i<self.scrollView.subviews.count; i++) {
        ...
        imageView.tag = index;
        FAShoppingDetailPhotoModel * model = [_images objectAtIndex:index];
        [imageView sd_setBackgroundImageWithURL:[NSURL URLWithString:model.img] forState:UIControlStateNormal placeholderImage:nil];
    }
    // 设置偏移量在中间
    if (self.isScrollDirectionPortrait) {
