...
@interface FABalanceViewController ()<UITableViewDataSource,UITableViewDelegate,FABalanceViewDelegate>
{
...
@implementation FABalanceViewController
...
{
    static NSString *cellIdentify = @"FABalanceTableViewCell";
    
    FABalanceTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentify];
    if (!cell) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"FABalanceTableViewCell" owner:nil options:nil] lastObject];
...
-(FABalanceView *)balanceView{
    if (!_balanceView) {
        ...
        _balanceView.titleArr = @[@"余额",@"提现",@"退款"];
    }
    return _balanceView;
}
