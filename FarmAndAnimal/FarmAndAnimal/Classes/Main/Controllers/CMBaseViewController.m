...
- (void)customNavigationRighItem
{
    ...
    UIButton * rightButton = [CMCustomViewFactory  createButton:CGRectMake(Num_Zero, Num_Zero, Ten*6, Ten*3) title:titleStr Image:[UIImage imageNamed:@"icon_rightItemBg"]];
...
-(void)dontHaveShow:(NSString *)textStr withPic:(NSString *)picStr
{
    UIImageView * tipImg = [[UIImageView alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/2-150/2, SCREEN_HEIGHT/2-32-60-80, 150, 150)];
    tipImg.image = ImageNamed(picStr);
    tipImg.tag = kPicTag ;
    [self.view addSubview:tipImg];
    UILabel * messageLabel = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/2-200/2, tipImg.frame.origin.y+tipImg.frame.size.height-10, 200, 80)];
    messageLabel.numberOfLines = 0;
    messageLabel.tag = kMessageTag;
    messageLabel.textAlignment = NSTextAlignmentCenter;
    messageLabel.text = textStr;
    messageLabel.textColor = RGB(160, 160, 160);
    messageLabel.font = [UIFont fontWithName:@"Helvetica Neue" size:16.0f];
    [self.view addSubview:messageLabel];
}
-(void)haveHidden
{
    UILabel * tipLabel = (UILabel *)[self.view viewWithTag:kMessageTag];
    UIImageView * tipImg = (UIImageView *)[self.view viewWithTag:kPicTag];
    [tipLabel removeFromSuperview];
    [tipImg removeFromSuperview];
}
